	...
	protected static boolean isSetExpression(final List<Class<? extends IToken>> tokens) {
		...
		for (Class<? extends IToken> token : tokens) {
			if (token.equals(BracketRoundOpen.class) || token.equals(BracketSquareOpen.class)) {
				...
				bracketCount--;
			} else if (bracketCount == 0 && (token.equals(Union.class) || token.equals(Intersect.class) || token.equals(Substraction.class))) {
				return true;
			}
		}
		return false;
		...
		for (Class<? extends IToken> token : tokens) {
			if (token.equals(BracketRoundOpen.class) || token.equals(BracketSquareOpen.class)) {
				...
				bracketCount--;
					...
					&& (token.equals(Plus.class) || token.equals(Minus.class) || token.equals(Modulo.class) || token.equals(Div.class) || (token.equals(Star.class) && !DoubleColon.class.equals(last) && !Axis.class.equals(last) && !BracketRoundOpen.class.equals(last)))) {
				return true;
			}
			last = token;
		}
