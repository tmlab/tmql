 ...
 * 
 */
public class GetSubtypes extends FunctionImpl {

	public static final String GetSubtypes = "fn:get-subtypes";
	...
	public QueryMatches interpret(ITMQLRuntime runtime, IContext context,
		...
		for (Map<String, Object> tuple : parameters) {
			...
			if (oTopics instanceof Collection<?>){
				supertypes.addAll(index.getSubtypes((Collection<? extends Topic>)oTopics));
			}
			/*
			 * is topic
			 */
			else if ( oTopics instanceof Topic ){
				supertypes.addAll(index.getSubtypes((Topic)oTopics));
			}
		}
		/*
		 * return result
