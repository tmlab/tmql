...
public class Navigation extends ExpressionImpl {
	 ...
	 */
	public Navigation(IExpression parent,
		...
		IParserUtilsCallback callback = new IParserUtilsCallback() {
			...
			public void newToken(List<Class<? extends IToken>> tmqlTokens,
				 ...
				 */
				if (token.equals(MoveBackward.class)
						|| token.equals(MoveForward.class)) {
					checkForExtensions(Step.class, tmqlTokens, tokens, runtime);
				}
				/*
				 * special handling for non-canonical axis
				 */
				else if (NonCanonicalUtils.isNonCanonicalProduction(tmqlTokens,
						tokens)) {
					NonCanonicalUtils.toCanonical(runtime, Navigation.this,
							tmqlTokens, tokens);
				}
			}
		};
