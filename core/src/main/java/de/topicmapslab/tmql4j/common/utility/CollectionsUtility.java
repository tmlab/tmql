...
public final class CollectionsUtility {
	 ...
	 *         otherwise.
	 */
	public static boolean isContentEqual(final Collection<?> collectionA,
	 ...
	 */
	public static boolean retainAll(ITMQLRuntime runtime,
			ITupleSequence<Object> sequenceA,
			final ITupleSequence<Object> sequenceB) {

		/*
		 * create temporary sequences
		 */
		ITupleSequence<Object> sequenceA_;
		ITupleSequence<Object> sequenceB_;

		sequenceA_ = runtime.getProperties().newSequence();
		sequenceB_ = runtime.getProperties().newSequence();
	...
	public static <E extends Object> ITupleSequence<E> atomify(
			final ITMQLRuntime runtime, ITupleSequence<E> sequence) {
		/*
		...
		if (sequence.isEmpty()) {
			 ...
			 * create new sequence
			 */
			ITupleSequence<E> seq;

			seq = runtime.getProperties().newSequence();
			...
			for (E obj : sequence) {
				 ...
				 */
				if (obj instanceof ITupleSequence<?>) {
					seq.addAll(atomify(runtime, (ITupleSequence<E>) obj));
				}
				/*
	 ...
	 *         otherwise
	 */
	private static <E extends Object> boolean hasToAtomify(
			ITupleSequence<E> sequence) {
		if (sequence.isEmpty()) {
			return false;
		}
		for (E obj : sequence) {
	 ...
	 */
	public static final boolean containsAll(Map<String, Object> map,
			Map<String, Object> containment, final Map<String, String> mapping) {
		/*
		...
		for (Entry<String, Object> entry : containment.entrySet()) {
			...
			if (map.containsKey(key)
					|| entry.getKey().equalsIgnoreCase("fn:count")) {
