 ...
 *
 */
package de.topicmapslab.tmql4j.draft2011.path.grammar.functions.url;
		...
		for (Map<String, Object> tuple : parameters) {
			...
			if (sequence instanceof Collection<?>) {
				...
				for (Object obj : (Collection<?>) sequence) {
					...
					if (value != null) {
						try {
							locs.add(URLDecoder.decode(obj.toString(), "UTF-8"));
			 ...
			 */
				...
				try {
					...
					if (value != null) {
						result.put(QueryMatches.getNonScopedVariable(), URLDecoder.decode(value, "UTF-8"));
					}
				} catch (UnsupportedEncodingException e) {
					e.printStackTrace();
				}
