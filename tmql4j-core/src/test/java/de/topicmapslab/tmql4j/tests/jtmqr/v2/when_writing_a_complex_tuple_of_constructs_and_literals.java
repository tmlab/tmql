...
public class when_writing_a_complex_tuple_of_constructs_and_literals extends with_TopicMap {
	...
	public void it_should_write_JTM_construct_nodes() {
		...
		assertEquals("name", tupleNode.get(2).get("jtm").get("item_type").getTextValue());
	}

	@Test
	public void it_should_write_literal_nodes() {
		assertEquals(getClass().getName(), tupleNode.get(1).getTextValue());
		assertEquals(getClass().getSimpleName(), tupleNode.get(3).getTextValue());
	}
}
