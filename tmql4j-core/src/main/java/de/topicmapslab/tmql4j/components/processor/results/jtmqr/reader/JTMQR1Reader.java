...
public class JTMQR1Reader {
	...
	protected IResult handleField(SimpleJtmqrResultSet resultSet, IResult result, String text) throws JsonParseException, IOException {
		...
		if (text.equals(IJtmQrKeys.ALIASES)) {
			 ...
			 * read aliases
			 */
			Map<String, Integer> aliases = JTMQRConstructReader.readAliases(this.jParser);
	...
	protected Object getValueFromField(String text) throws JsonParseException, IOException {
		 ...
		 */
			...
			if (text.equals(IJTMConstants.TOPIC)) {
				return JTMQRConstructReader.readTopic(this.jParser);
			}
			/*
			 * is name
			 */
			else if (text.equals(IJTMConstants.NAME)) {
				return JTMQRConstructReader.readName(this.jParser, null);
			}
			/*
			 * is occurrence
			 */
			else if (text.equals(IJTMConstants.OCCURRENCE)) {
				return JTMQRConstructReader.readOccurrence(this.jParser, null);
			}
			/*
			 * is association
			 */
			else if (text.equals(IJTMConstants.ASSOCIATION)) {
				return JTMQRConstructReader.readAssociation(this.jParser);
			}
			/*
			 * is role
			 */
			else if (text.equals(IJTMConstants.ROLE)) {
				return JTMQRConstructReader.readRole(this.jParser, null);
			}
			/*
			 * is variant
			 */
			else if (text.equals(IJTMConstants.VARIANT)) {
				return JTMQRConstructReader.readVariant(this.jParser, null);
			}
		}
		/*
		 * is array of values
