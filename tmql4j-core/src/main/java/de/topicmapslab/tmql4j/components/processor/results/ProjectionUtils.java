...
public class ProjectionUtils {
	 ...
	 * @return the result tuples
	 */
	public static List<Map<String, Object>> asTwoDimensionalMap(QueryMatches matches) {
		List<Map<String, Object>> results = HashUtil.getList();
		List<String> keys = matches.getOrderedKeys();
		for (Map<String, Object> tuple : matches) {
			Map<String, Object> current = HashUtil.getHashMap();
			asTwoDimensionalMap(tuple, keys, current, results);
		}
		return results;
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static void asTwoDimensionalMap(Map<String, Object> tuple, List<String> keys, Map<String, Object> current, List<Map<String, Object>> results) {
		/*
		 * get current key
		 */
		final String key = keys.get(0);
		...
		if (value instanceof Collection<?>) {
			for (Object val : CollectionsUtility.asTwoDimension((Collection<Object>) value)) {
				Map<String, Object> current_ = HashUtil.getHashMap(current);
				current_.put(key, val);
				if (keys_.isEmpty()) {
					results.add(current_);
				} else {
					asTwoDimensionalMap(tuple, keys_, current_, results);
				}
			}
		} else {
			Map<String, Object> current_ = HashUtil.getHashMap(current);
			current_.put(key, value);
			if (keys_.isEmpty()) {
				results.add(current_);
			} else {
				asTwoDimensionalMap(tuple, keys_, current_, results);
