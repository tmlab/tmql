 ...
 * 
 */
public class ZagZigFunctionTranslator extends FunctionTranslatorImpl {
	...
	protected ISelection getSelection(ISqlDefinition definition, List<ISqlDefinition> parameters, List<IFromPart> fromParts) {
		...
		/*
		 * is a flat sequence of tuples
		 */
		SqlTables currentTable = set.getLastSelection().getCurrentTable();
		String content = set.toString();
		if (currentTable.isRecord()) {
			content = set.getLastSelection().getColumn();
			content = content.substring(14, content.length() - 3);
			currentTable = currentTable.rawOf();
		}
		IFromPart fromPart = new FromPart(content, definition.getAlias(), false);
		definition.addFromPart(fromPart);
		ISelection selection = new Selection(ISqlConstants.ANY, null, true);
		selection.setCurrentTable(currentTable);
		return selection;
	}
