...
public class SimpleContentTranslator extends TmqlSqlTranslatorImpl<SimpleContent> {
	...
	public IState transform(ITMQLRuntime runtime, IContext context, IExpression expression, IState state) throws TMQLRuntimeException {
		...
		IState anchorState = TranslatorRegistry.getTranslator(Anchor.class).transform(runtime, context, expression.getExpressions().get(0), state);
		return TranslatorRegistry.getTranslator(Navigation.class).transform(runtime, context, expression.getExpressions().get(1), anchorState);
	}
