...
public class TestGroupByClause extends Tmql4JTestCase {
	...
	public void testSingleGroupByVariable() throws Exception {
		...
		String query = "RETURN myTopic , myTopic >> types";
		...
		query = "GROUP BY $0 RETURN myTopic , myTopic >> types";
		set = execute(query);
	...
	public void testSingleGroupByVariable2() throws Exception {
		...
		String query = "RETURN myTopic >>  types, myTopic";
		SimpleResultSet set = execute(query);
		assertEquals(100, set.size());
		for (IResult result : set) {
			assertEquals(2, result.size());			
		...
		query = "GROUP BY $1 RETURN myTopic >>  types, myTopic";
		set = execute(query);
		assertEquals(1, set.size());
		assertEquals(2, set.first().size());
	...
	public void testInvalidGroupByVariable() throws Exception {
		...
		String query = "GROUP BY $3 RETURN myTopic >>  types, myTopic";
		execute(query);
	}
	...
	public void testDoubleGroupByVariable() throws Exception {
		...
		String query = "RETURN myTopic, myTopic >> indicators, myTopic >> types";
		SimpleResultSet set = execute(query);
		...
		query = "GROUP BY $0 RETURN myTopic, myTopic >> indicators, myTopic >> types";
		set = execute(query);
		assertEquals(1, set.size());
		...
		query = "GROUP BY $0, $1 RETURN myTopic, myTopic >> indicators, myTopic >> types";
		set = execute(query);
