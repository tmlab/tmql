...
public class TestPreparedStatement extends Tmql4JTestCase {
	...
	public void testOffsetLimitClause() throws Exception {
		...
		IPreparedStatement stmt = runtime.preparedStatement("WHERE $var AKO myType ORDER BY $var / tm:name [0] OFFSET ? LIMIT 10  RETURN $var");
		...
		stmt = runtime.preparedStatement("WHERE $var AKO myType ORDER BY $var / tm:name [0] DESC OFFSET ? LIMIT ? RETURN $var");
		stmt.setTopicMap(topicMap);
		for (int index = 0; index < 10; index++) {
			stmt.setLong(0, index * 10);
			stmt.setLong(1, 10);
