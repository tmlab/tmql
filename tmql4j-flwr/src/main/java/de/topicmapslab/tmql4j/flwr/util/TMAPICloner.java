...
public class TMAPICloner {
	...
	public TopicMap clone(Collection<?> sequence) throws Exception {
		...
		for (Object obj : variants) {
			...
			clone(clone(clone(variant.getParent().getParent()), variant.getParent()), variant);
		}
	...
	public Occurrence clone(Topic topic, Occurrence occurrence) throws Exception {

		Topic type = clone(occurrence.getType());

		Occurrence clone = topic.createOccurrence(type, occurrence.getValue(), map.createLocator(occurrence.getDatatype().getReference()), new Topic[0]);
	...
	public Name clone(Topic topic, Name name) throws Exception {
		...
		if (type == null) {
			try {
				type = topic.getTopicMap().getTopicBySubjectIdentifier(topic.getTopicMap().createLocator(TmdmSubjectIdentifier.TMDM_DEFAULT_NAME_TYPE));
				if (type == null) {
					topic.getTopicMap().createTopicBySubjectIdentifier(topic.getTopicMap().createLocator(TmdmSubjectIdentifier.TMDM_DEFAULT_NAME_TYPE));
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public Variant clone(Name name, Variant variant) throws Exception {
		...
		Variant clone = name.createVariant(variant.getValue(), map.createLocator(variant.getDatatype().getReference()), new Topic[0]);
