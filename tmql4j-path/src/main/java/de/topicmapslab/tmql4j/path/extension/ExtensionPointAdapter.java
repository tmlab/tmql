 ...
 *
 */
package de.topicmapslab.tmql4j.path.extension;
...
public class ExtensionPointAdapter implements IExtensionPointAdapter {
	...
	public void loadExtensionPoints() throws TMQLExtensionRegistryException {
		...
		if (pointList != null && pointList.iterator().hasNext()) {

			for (IExtensionPoint extensionPoint : pointList) {
				if (extensionPoints.containsKey(extensionPoint.getExtensionPointId())) {
					throw new TMQLExtensionRegistryException("Duplicate extension point id '" + extensionPoint.getClass().getName() + " and "
							+ extensionPoints.get(extensionPoint.getExtensionPointId()));
				}
				/*
				 * register extension point first
				 */
				 ...
				 */
				registeredLanguageExtenstions.put(extensionPoint, extensionPoint.getLanguageExtensions());
				for (ILanguageExtension languageExtension : extensionPoint.getLanguageExtensions()) {
					Set<ILanguageExtension> set = languageExtensions.get(languageExtension.getExpressionType());
					if (set == null) {
						set = HashUtil.getHashSet();
						languageExtensions.put(languageExtension.getExpressionType(), set);
					}
					set.add(languageExtension);
				}
			}
		}
	}
