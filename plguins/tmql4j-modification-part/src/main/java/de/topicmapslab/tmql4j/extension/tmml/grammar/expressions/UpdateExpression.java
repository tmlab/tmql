...
public class UpdateExpression extends ExpressionImpl {
	 ...
	 */
	public UpdateExpression(IExpression parent,
		...
		if (!runtime.getProperties().isLanguageExtensionTmqlUlEnabled()) {
			throw new TMQLGeneratorException(
					"Keyword 'UPDATE' not allowed at current TMQL session, because the TMQL-UL extension is disabled.");
		}
		...
		IParserUtilsCallback callback = new IParserUtilsCallback() {
			...
			public void newToken(List<Class<? extends IToken>> tmqlTokens,
				...
				if (foundDelimer == null && containsWhere) {
					 ...
					 */
					checkForExtensions(UpdateClause.class, tmqlTokens, tokens,
							runtime);
				}
			}
		};
	...
	public boolean isValid() {
		if (getTmqlTokens().isEmpty()) {
			...
			return false;
		} else if (!getTmqlTokens().contains(Set.class)
