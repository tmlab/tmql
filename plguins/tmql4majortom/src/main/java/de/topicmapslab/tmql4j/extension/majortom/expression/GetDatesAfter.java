 ...
 * 
 */
public class GetDatesAfter extends ExpressionInterpreterImpl<FunctionInvocation> implements IFunctionInvocationInterpreter<FunctionInvocation> {

	public static final String GetDatesAfterIdentifier = "fn:dates-after";

	/**
	 * @param ex
	 */
	public GetDatesAfter(FunctionInvocation ex) {
		super(ex);
	}
		...
		for (Map<String, Object> tuple : parameters) {
			try {
				...
				for (ICharacteristics c : index.getCharacteristics(runtime.getTopicMap().createLocator(XmlSchemeDatatypes.XSD_DATETIME))) {
					if ( ((IOccurrence)c).dateTimeValue().after(calendar)){
	...
	public String getItemIdentifier() {
		return GetDatesAfterIdentifier;
	}
