...
public class TestBestLabel extends Tmql4JTestCase {
	...
	public void smalltest() {
		...
		System.out.println(execute("Select fn:best-label( // tm:subject )"));
		
		
	}

	private String getBestLabel(Topic topic) {
		String queryString = "Select fn:best-label( \"" + topic.getId() + "\" << id )";
	...
	private String getBestLabel(Topic topic, Topic theme) {
		String queryString = "Select fn:best-label( \"" + topic.getId() + "\" << id, \"" + theme.getId() + "\" << id )";
		SimpleResultSet rs = execute(queryString);
		if (rs.isEmpty())
			return null;
		return rs.get(0, 0);
	...
	private String getBestLabel(Topic topic, Topic theme, boolean strict) {
		String queryString = "Select fn:best-label( \"" + topic.getId() + "\" << id, \"" + theme.getId() + "\" << id, \"" + Boolean.toString(strict) + "\" )";
		SimpleResultSet rs = execute(queryString);
		if (rs.isEmpty())
			return null;
